# AI-Career Companion Agent for Internship Matching & Interview Preparation — System Documentation & Technical Blueprint

A comprehensive architectural and implementation reference for the **AI-Career Companion Agent for Internship Matching & Interview Preparation** — an end-to-end, AI-powered career development platform featuring automated resume parsing, multi-stage semantic internship matching, AI cover letter generation, a grounded RAG career advisor, an interactive multi-turn interview preparation agent, and a multilingual voice-first resume architect with executive ATS PDF generation.
...
10. [Deployment Guide (Railway + Vercel)](#10-deployment-guide-railway--vercel)

---

## 1. Executive Overview

Modern career readiness platforms often suffer from high barriers to entry: complex resume builders, rigid upload requirements, language barriers, and lack of personalized interview guidance. The **AI-Career Companion Agent for Internship Matching & Interview Preparation** addresses these challenges through a unified platform with the following core pillars:
...
- **Resume Upload Flow**: User uploads PDF/DOCX → File magic bytes validated → Text extracted → Regex & taxonomy extract skills/metadata → Saved to database → Embeddings generated → Top internship matches queried from FAISS → Returned to client.
- **Voice Resume Flow**: User speaks in Telugu → Browser Web Speech API captures speech in continuous mode → Spoken transcript sent to `/voice-resume/extract` → Groq LLM translates to fluent English, fixes grammar, and extracts structured fields → Client displays real-time transcript & extracted profile → User clicks "Build Resume Now" → `/voice-resume/generate` formats ATS resume in English → Client exports formatted PDF via `jsPDF`.
...
- **Files**: `app/auth.py`, `app/dependencies.py`, `app/routers/auth.py`
...
- **Files**: `app/parser.py`, `app/resume_parser.py`, `app/routers/resume.py`
...
- **Files**: `app/candidate_preprocessor.py`, `app/internship_index.py`, `app/llm_ranker.py`
...
- **Files**: `app/cover_letter_service.py`, `app/routers/cover_letter.py`
- **Prompt Engineering**:
...
- **Files**: `app/rag/ingest.py`, `app/rag/retriever.py`, `app/rag/generator.py`, `app/routers/career_assistant.py`
...
- **Files**: `app/interview_agent_service.py`, `app/routers/interview_agent.py`
- **Capabilities**:
...
- **Files**: `app/voice_resume_service.py`, `app/routers/voice_resume.py`, `frontend/src/utils/resumePdf.ts`
- **Multilingual Input → 100% English Output**:
...
- **Client-Side PDF Generation (`jsPDF`)**:
  - `frontend/src/utils/resumePdf.ts` renders executive typography with bold section headers, underline rules, bullet indentations, and multi-page pagination.
...
- `POST /auth/token`: OAuth2 password request → `{ "access_token": "...", "token_type": "bearer" }`.
...
- `POST /cover-letter/generate`: Input `{ company_name, job_role, job_description, resume_id? }` → `{ cover_letter: "..." }`.
- `GET /cover-letter/history`: Fetch user's previous cover letters.

### Career Assistant RAG (`/career-assistant`)
- `POST /career-assistant/chat`: Input `{ message, conversation_history? }` → `{ reply: "...", sources: [...] }`.
...
### App Layout & Routing
- `frontend/src/App.tsx` uses React Router v7:
...
- **API Client**: `frontend/src/api/client.ts` provides a unified `request<T>()` fetch wrapper that automatically appends `Authorization: Bearer <token>` and parses backend errors.
- **Theme System**: Responsive dark mode managed via CSS variables (`var(--accent)`, `var(--surface)`, `var(--text)`, `var(--border)`) — premium obsidian/indigo palette.
- **Audio & Speech Engine**:
  - `SpeechRecognitionAPI`: Web Speech API with `continuous: true` and a 3.5s auto-silence timer for mic recording in Interview Agent.
  - `SpeechSynthesis`: Text-to-speech for speaking agent replies aloud (muted by default; user-activated per message).
  - `AudioContext` & `OfflineAudioContext`: Decodes video audio tracks client-side for voice resume.
- **PDF Generation**:
  - `frontend/src/utils/resumePdf.ts` creates executive A4 PDFs with 40pt margins, bold headers, divider lines, and text wrapping.
...
### Step 5: Initialize Frontend Environment (Node.js 18+)
```bash
cd frontend
...
3. Railway auto-detects Python via Nixpacks using [`railway.json`](railway.json).
4. Set the following environment variables in Railway dashboard:
   - `GROQ_API_KEY`
   - `SECRET_KEY`
   - `DATABASE_URL` (use Railway's PostgreSQL plugin for production, or leave blank for SQLite)
...
5. [`frontend/vercel.json`](frontend/vercel.json) handles SPA routing — all paths rewrite to `/index.html`.
